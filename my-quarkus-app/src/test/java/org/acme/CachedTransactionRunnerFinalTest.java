...
/**
 * Final comprehensive test for CachedTransactionRunner that demonstrates the complete caching flow.
 * This test properly simulates the real-world scenario where:
 * 1. First call: Input has no TF fields, but response has TF fields (gets cached)
 * 2. Second call: Uses cache key from first call to hydrate fields
 * 
 * The test uses in-memory cache for both FieldHydrationService and ResponseCacheServiceImpl
 * to avoid cache mismatch issues.
 */
@QuarkusTest
@TestProfile(CachedTransactionRunnerFinalTest.TestProfile.class)
public class CachedTransactionRunnerFinalTest {
    ...
    /**
     * Test profile to enable caching for this test and use in-memory cache for both services
     */
