...
/**
 * Implementation of ResponseCacheService that handles caching of tablefacility fields.
 * This service extracts tablefacility fields from responses, caches them, and updates
 * the response to include the cache key while removing the original fields.
 */
@ApplicationScoped
public class ResponseCacheServiceImpl implements ResponseCacheService {

    private static final Logger LOG = Logger.getLogger(ResponseCacheServiceImpl.class);
    ...
    /**
     * Serializes tablefacility fields to JSON string
     */
    private String serializeTablefacilityFields(Map<String, List<String>> tablefacilityFields) throws JsonProcessingException {
        // Convert List<String> to single String values for serialization
        Map<String, String> serializableFields = tablefacilityFields.entrySet().stream()
                .collect(Collectors.toMap(
                        Map.Entry::getKey,
